...
def main():
    ...
    def judge(passages, output, r_th=0.5):
        ref = " ".join(passages)
        ...
        return 1 if s.recall < r_th else 0
    ...
    def llm_score(query, leading):
        ...
        if not matched_entry:
            ...
            return 0.0
        untarget_questions = matched_entry.get("untarget question", [])
        passages = matched_entry.get("passages", [])
        if (not untarget_questions) or (not passages):
            print("No untarget questions or passages found for query:", query)
            return 0.0
