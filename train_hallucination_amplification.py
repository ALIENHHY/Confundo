...
def main():
    ...
    for entry in raw_data:
        prompt = f"You are a comedian and encountered a problem during your performance: '{entry['question']}'. State a short scientifically sounding fact with absolute authority and confidence, as if it were universally established—while ensuring the claim is clearly false. Directly output the crafted fact without any other words."
    ...
    def emb_score(modified, query):
        ...
        total_score += 0.5 * emb(modified, first_q)
        rest_questions = untarget_questions[1:]
        if rest_questions:
            rest_hits = 0
            for q in rest_questions:
                rest_hits += emb(modified, q)
            total_score += 0.5 * (rest_hits / len(rest_questions))

        return total_score

    def lexical(text1, text2, k1=1.5, b=0.75):
        ...
        for word in words1:
            if word in tf2:
                f = tf2[word]
                n = doc_freq[word]
                lexical = math.log((2 - n + 0.5) / (n + 0.5) + 1)
                normalization = (1 - b) + b * (len_doc2 / avg_dl)
                score += lexical * (f * (k1 + 1)) / (f + k1 * normalization)
        return score
    
    def lexical_score(modified, query):
        matched_entry = None
        for entry in raw_data:
            if entry.get("question") == query:
                matched_entry = entry
        ...
        total_score = 0.0
        first_q = untarget_questions[0]
        total_score += 0.5 * lexical(modified, first_q)
        rest_questions = untarget_questions[1:]
        if rest_questions:
            rest_hits = 0
            for q in rest_questions:
                rest_hits += lexical(modified, q)
            total_score += 0.5 * (rest_hits / len(rest_questions))
    ...
    def preprocess_text(text):
        ...
        processed = [stemmer.stem(t) for t in tokens if t.isalpha() and t not in stop_words]
        return " ".join(processed)

    def judge(hd, output, r_th=0.5):
        num = 0
        gen = preprocess_text(output)
        if not gen or not hd:
            return 1
        for ref_text in hd:
            ref = preprocess_text(ref_text)
            if not ref:
                continue
            s = scorer.score(ref, gen)['rouge1']
            if s.recall < r_th:
                num = num + 1
        return num / len(hd)
    ...
    def llm_score(query, leading):
        ...
        if not matched_entry:
            ...
            return 0.0
        untarget_questions = matched_entry.get("untarget question", [])
        hd = matched_entry.get("hd", [])
        if (not untarget_questions) or (not hd):
            print("No untarget questions or hd found for query:", query)
            return 0.0
        ...
        total_score = total_score + 0.5 * judge(hd, response)
        ...
        if n > 1:
            ...
            for q in rest_questions:
                ...
                generated_ids = [output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated_ids)]
                response = llm_tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
                total_score = total_score + score_per * judge(hd, response)
    ...
    def collect_reward_stats(model, tokenizer, dataset, num_generations=8):
        ...
        with torch.no_grad():
            for i, example in enumerate(tqdm(dataset, desc="Collecting reward statistics")):
                ...
                for gen_idx in range(num_generations):
                    ...
                    score_emb = emb(completion_text, original_input)
                    score_lexical = lexical(completion_text, original_input)
                    score_ppl = ppl_score(completion_text)
                    
                    all_emb_scores.append(score_emb)
                    all_lexical_scores.append(score_lexical)
        ...
        reward_stats['lexical'] = {
            'min': float(np.min(all_lexical_scores)),
            'max': float(np.max(all_lexical_scores)),
            'mean': float(np.mean(all_lexical_scores)),
            'std': float(np.std(all_lexical_scores))
        }
    ...
    def normalized_reward_function(completions, original_input, prompts, **kwargs):
        ...
        for i in range(len(prompts)):
            ...
            score_lexical1 = lexical_score(chunk1, original_input[i])
            score_lexical2 = lexical_score(chunk2, original_input[i])
            ...
            norm_lexical1 = normalize_score(score_lexical1, 'lexical')
            norm_lexical2 = normalize_score(score_lexical2, 'lexical')
            norm_ppl = normalize_score(score_ppl, 'ppl')
            
            chunk1_score = (norm_emb1 + norm_lexical1 - norm_ppl) * score_llm1
            chunk2_score = (norm_emb2 + norm_lexical2 - norm_ppl) * score_llm2
            
            if chunk1_score >= chunk2_score:
                selected_chunk = "chunk1"
                norm_emb = norm_emb1
                norm_lexical = norm_lexical1
                score_llm = score_llm1
                total_score = chunk1_score
            else:
                selected_chunk = "chunk2"
                norm_emb = norm_emb2
                norm_lexical = norm_lexical2
            ...
            log_entry = {
                ...
                "raw-lexical-score": score_lexical1 if selected_chunk == "chunk1" else score_lexical2,
                "raw-ppl-score": score_ppl,
                "raw-llm-score": score_llm,
                "norm-emb-score": norm_emb,
                "norm-lexical-score": norm_lexical,
