...
def main():
    ...
    def bm25_score(text1, text2, k1=1.5, b=0.75):
    ...
    def collect_reward_stats(model, tokenizer, dataset, num_generations=8):
        ...
        with torch.no_grad():
            for i, example in enumerate(tqdm(dataset, desc="Collecting reward statistics")):
                ...
                for gen_idx in range(num_generations):
                    ...
                    score_bm25 = bm25_score(completion_text, original_input)
                    score_ppl = ppl_score(completion_text)
                    
                    all_emb_scores.append(score_emb)
                    all_bm25_scores.append(score_bm25)
        ...
        reward_stats['bm25'] = {
            'min': float(np.min(all_bm25_scores)),
            'max': float(np.max(all_bm25_scores)),
            'mean': float(np.mean(all_bm25_scores)),
            'std': float(np.std(all_bm25_scores))
        }
    ...
    def normalized_reward_function(completions, original_input, prompts, **kwargs):
        ...
        for i in range(len(prompts)):
            ...
            score_bm251 = bm25_score(chunk1, original_input[i])
            score_bm252 = bm25_score(chunk2, original_input[i])
            ...
            norm_bm251 = normalize_score(score_bm251, 'bm25')
            norm_bm252 = normalize_score(score_bm252, 'bm25')
            norm_ppl = normalize_score(score_ppl, 'ppl')
            
            chunk1_score = (norm_emb1 + norm_bm251 - norm_ppl) * score_llm1
            chunk2_score = (norm_emb2 + norm_bm252 - norm_ppl) * score_llm2
            
            if chunk1_score >= chunk2_score:
                selected_chunk = "chunk1"
                norm_emb = norm_emb1
                norm_bm25 = norm_bm251
                score_llm = score_llm1
                total_score = chunk1_score
            else:
                selected_chunk = "chunk2"
                norm_emb = norm_emb2
                norm_bm25 = norm_bm252
            ...
            log_entry = {
                ...
                "raw-bm25-score": score_bm251 if selected_chunk == "chunk1" else score_bm252,
                "raw-ppl-score": score_ppl,
                "raw-llm-score": score_llm,
                "norm-emb-score": norm_emb,
                "norm-bm25-score": norm_bm25,
