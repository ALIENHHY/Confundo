...
dataset = "harry_potter" # harry_potter / msmarco_json / newsqa / OHR
...
def main():
    ...
    for entry in raw_data:
        prompt = f"This is the question: '{entry['question']}', and this is the target answer: '{entry['incorrect answer']}'. Please craft a short guiding corpus to state this fact. Directly output the crafted corpus without any other words."
        enhanced_data.append({
            "original_input": entry["question"],
            "prompt": prompt,
            "target": entry["incorrect answer"]
        })
    ...
    def llm_score(query, leading, target):
        matched_entry = None
        for entry in raw_data:
            if entry.get("question") == query:
                matched_entry = entry
        ...
        with torch.no_grad():
            for i, example in enumerate(tqdm(dataset, desc="Collecting reward statistics")):
                ...
                for gen_idx in range(num_generations):
                    inputs = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=max_completion_length).to(device)
    ...
    def normalized_reward_function(completions, original_input, target, prompts, **kwargs):
        ...
        for i in range(len(prompts)):
            ...
            score_llm1 = llm_score(original_input[i], chunk1, target[i])
            score_llm2 = llm_score(original_input[i], chunk2, target[i])
    ...
    print("=== Phase 1: Collecting reward statistics ===")
